...
def get_course_detail(csr,org,longname):
     ...
     for course_det in collection.find({"_id.course":csr,"_id.org":org,"_id.category":"course"},{"metadata.start":1,"metadata.end":1,"metadata.enrollment_start":1,"metadata.enrollment_end":1,"metadata.course_image":1,"metadata.display_name":1,"definition.data.grading_policy":1}):
           ...
           try:
                   ...
                   course_end= dateutil.parser.parse(temp)
           except:
                   course_end=dateutil.parser.parse("4712-12-31T00:00:00Z")
           try:
                   course_enroll_end=course_det["metadata"]["enrollment_end"]
                   course_enroll_end=dateutil.parser.parse(course_enroll_end)
           except:
                   course_enroll_end=course_end
           try: 
                   course_start=course_det["metadata"]["start"]
                   course_start=dateutil.parser.parse(course_start)
           except:
                   course_start= course_end + timedelta(days=-1)
           try: 
                   course_enroll_start=course_det["metadata"]["enrollment_start"]
                   course_enroll_start=dateutil.parser.parse(course_enroll_start)
           except:
                   course_enroll_start= course_start + timedelta(days=-1)      
  ...
  for mod in mlist:
       ...
       for moddetails in collection.find({"_id.course":csr,"_id.org":org,"_id.category":mtype,"_id.name":mid},{"metadata.display_name":1,"definition.children":1,"_id.name":1, "metadata.graded":1,"metadata.visible_to_staff_only":1,"metadata.format":1,"metadata.start":1,"metadata.due":1,"definition.data.data":1,"metadata.weight":1}):
            ...
            try:
               ...
               due_date=dateutil.parser.parse(moddetails["metadata"]["due"])
            except:
               due_date=end
            try:
               release_date= dateutil.parser.parse(moddetails["metadata"]["start"])
...
def course_modules(course_id,org,cid):
    ...
    for csr_name in collection.find({"_id.course":csr,"_id.org":org,"_id.category":"course"} ,{"metadata.display_name":1, "metadata.visible_to_staff_only":1,"definition.children":1,"metadata.start":1,"metadata.end":1}): 
         ...
         try:
            ...
            course_end=dateutil.parser.parse("4712-12-31T00:00:00Z")
         try: 
                   course_start=dateutil.parser.parse(csr_name["metadata"]["start"])
         except:
                   course_start= course_end + timedelta(days=-1)
         ...
         if (course_id != "-1") :
           try:
             ...
             try:
                ...
                ahead_date=dateutil.parser.parse("4712-12-31T23:00:00Z")
             try:
                coursestart=course_obj.coursestart
             except:
                coursestart=dateutil.parser.parse("1111-01-01T23:00:00Z")
             try:
                enrollstart=course_obj.enrollstart
             except:
                enrollstart=dateutil.parser.parse("1111-01-01T23:00:00Z")
             try:
                enrollend=course_obj.enrollend
             except:
                enrollend=dateutil.parser.parse("4712-12-31T23:00:00Z")
             if (enrollstart < curtime) and (curtime < (enrollend+ timedelta(days=num_days))):
                
                if course_obj.blended_mode==1:
                        status=get_student_course_enrollment(course_id)
             else:
               print course_id, " Enrollment is closed."
             if (coursestart < curtime) and (curtime < ahead_date):
             ...
             print course_id," doesnot exist in edxcourses.",str(e.message)
         else:
...
def main(argv):
        ...
        for course in ["IITBombayX/EE210xS16/2016_T1","IITBombayX/CS101.1xS16/2016_T1","IITBombayX/ME209xS16/2016_T1","IITBombayX/HS791xS16/2016_T1","IITBombayX/SKANI101x/2016_T1","IITBombayX/ET601Tx/2015-16","TISS/SKANI101x/2015-16"]:
